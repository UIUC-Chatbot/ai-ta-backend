...
class Affiliation:
  ...
  def as_json(self):
    return {"laboratory": self.laboratory, "institution": self.institution, "location": self.location}
...
class Paper:
  ...
  def __init__(self, paper_id: str, pdf_hash: str, metadata: Dict, abstract: List[Dict], body_text: List[Dict],
               back_matter: List[Dict], bib_entries: Dict, ref_entries: Dict):
...
def load_s2orc(paper_dict: Dict) -> Paper:
  ...
  elif ("pdf_parse" in paper_dict and paper_dict.get("pdf_parse")) or ("body_text" in paper_dict and
                                                                       paper_dict.get("body_text")):
