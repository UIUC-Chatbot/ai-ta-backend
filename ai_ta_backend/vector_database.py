...
Now please respond to my question: {user_question}"""
      ...
      with NamedTemporaryFile() as pdf_tmpfile:
        ...
        if kwargs['kwargs'] == {}:
          ...
          if 'base_url' in kwargs['kwargs'].keys():
            ...
            base_url = ''
          page['readable_filename'] = str(object=time.localtime()[1]) + "/" + str(time.localtime()[2]) + "/" + str(
              time.localtime()[0])[2:] + ' ' + page['readable_filename']
  ...
  def ingest_coursera(self, coursera_course_name: str, course_name: str) -> str:
    ...
    try:
      subprocess.run(f"coursera-dl {always_use_flags} {certificate} {coursera_course_name}",
                     check=True,
                     shell=True,
  ...
  def split_and_upload(self, texts: List[str], metadatas: List[Dict[str, Any]]):
    ...
    try:
      ...
      self.supabase_client.table(os.getenv('MATERIALS_SUPABASE_TABLE')).insert(data).execute()  # type: ignore
