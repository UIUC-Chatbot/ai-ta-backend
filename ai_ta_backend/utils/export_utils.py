...
def _initialize_excel(excel_file_path):
	...
	headers = ["Conversation ID", "User Email", "Course Name", "Message ID", "Timestamp", "User Role", "Message Content", "Had Images(Yes/No)"]
...
def _process_conversation(s3, convo, course_name, file_paths, worksheet, row_num, error_log, wrap_format):
	try:
		...
		_create_markdown(s3, convo_id, messages, file_paths['markdown_dir'], file_paths['media_dir'], user_email, error_log, timestamp, convo_name)
...
def _create_zip(file_paths, error_log):
	...
	with zipfile.ZipFile(zip_file_path, 'w', compression=zipfile.ZIP_DEFLATED) as zipf:
		for root, _, files in os.walk(file_paths['markdown_dir']):
			for file in files:
				zipf.write(os.path.join(root, file), os.path.join('markdown export', os.path.relpath(os.path.join(root, file), file_paths['markdown_dir'])))
		for root, _, files in os.walk(file_paths['media_dir']):
			for file in files:
				zipf.write(os.path.join(root, file), os.path.join(file_paths['media_dir'].split('/')[-1], os.path.relpath(os.path.join(root, file), file_paths['media_dir'])))
