...
class PubmedExtraction:

  def __init__(self):
    self.posthog_client = Posthog(sync_mode=False,
    ...
    self.minio_client = Minio(os.environ['MINIO_ENDPOINT'],
                              access_key=os.environ['MINIO_ACCESS_KEY'],
                              secret_key=os.environ['MINIO_SECRET'],
                              secure=True)

  def extractPubmedData(self):
    """
    ...
    file_list = self.getFileList(ftp_address, ftp_path, ".gz")
    ...
    self.posthog_client.capture(distinct_id="pubmed_extraction",
  ...
  def getFilesToProcess(self, file_list: list):
    last_processed_response = self.supabase_client.table("pubmed_daily_update").select("*").order(
  ...
  def processPubmedXML(self, file: str, ftp_address: str, ftp_path: str):
    """
        Main function to extract metadata and articles from the PubMed baseline folder.
        """
    start_time = time.monotonic()
    try:
      ...
      gz_filepath = self.downloadXML(ftp_address, ftp_path, file, "pubmed")
      ...
      for i, metadata in enumerate(self.extractMetadataFromXML(xml_filepath, destination_dir, error_log)):
        ...
        metadata_with_ids = self.getArticleIDs(metadata, error_log)
        ...
        complete_metadata = self.downloadArticles(metadata_with_ids, batch_dir, error_log)
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
          future = executor.submit(self.uploadToStorage, batch_dir, error_log)
      ...
      try:
        response = self.supabase_client.table("publications").upsert(final_metadata).execute()  # type: ignore
  ...
  def downloadXML(self, ftp_address: str, ftp_path: str, file: str, local_dir: str):
    """
  ...
  def getFileList(self, ftp_address: str, ftp_path: str, extension: str = ".gz"):
    """
  ...
  def extractXMLFile(self, gz_filepath: str):
    """
  ...
  def extractMetadataFromXML(self, xml_filepath: str, dir: str, error_file: str):
    """
    ...
    try:
      ...
      with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for item in article_items:
          future = executor.submit(self.processArticleItem, item, dir, error_file)
  ...
  def processArticleItem(self, item: ET.Element, directory: str, error_file: str):
    """
  ...
  def getArticleIDs(self, metadata: list, error_file: str):
    """
    ...
    for i in range(0, len(metadata), batch_size):
      ...
      try:
        response = requests.get(base_url + app_details + "&ids=" + ids, timeout=30)
        data = response.json()
        ...
        with Manager() as manager:
          ...
          with concurrent.futures.ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(self.updateArticleMetadata, shared_metadata, record): record for record in records
            }
  ...
  def updateArticleMetadata(self, shared_metadata, record):
    """
  ...
  def downloadArticles(self, metadata: list, dir: str, error_file: str):
    """
    ...
    try:
      ...
      download_article_partial = partial(self.download_article, api_url=base_url, dir=dir, error_file=error_file)
      with concurrent.futures.ProcessPoolExecutor() as executor:
  ...
  def download_article(self, article, api_url, dir, error_file):
    """
    ...
    try:
      ...
      if article['pmcid']:
        ...
        xml_response = requests.get(final_url, timeout=30)
        extracted_data = self.extractArticleData(xml_response.text, error_file)
        ...
        try:
          ...
          if filename.endswith(".tar.gz"):
            extracted_pdf_paths = self.extractPDF(local_file, dir, error_file, article['pmcid'])
  ...
  def extractPDF(self, tar_gz_filepath: str, dest_directory: str, error_file: str, pmcid: str):
    """
  ...
  def extractArticleData(self, xml_string: str, error_file: str):
    """
  ...
  def upload_file(self, client, bucket_name, file_path, object_name, error_file, upload_log):
    """
  ...
  def uploadToStorage(self, filepath: str, error_file: str):
    """
    ...
    try:
      ...
      for root, _, files_ in os.walk(filepath):
        for file in files_:
          ...
          files.append((self.minio_client, bucket_name, file_path, object_name, error_file, upload_log))

      with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        for i in range(0, len(files), 10):
          batch_files = files[i:i + 10]
          futures = [executor.submit(self.upload_file, *args) for args in batch_files]
