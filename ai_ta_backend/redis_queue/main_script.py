...
def update_task_status(job, connection, result, *args, **kwargs):
    ...
    try:
        ...
        if result_json['failure_ingest']:
            ...
            supabase_client.table('documents_in_progress').delete().eq('beam_task_id', job_id).execute()
...
def update_task_failure(job, exc_type, exc_value, traceback):
    ...
    try:
        ...
        supabase_client.table('documents_failed').insert(failure_payload).execute()
...
def queue_ingest_task(inputs):
    ...
    response = supabase_client.table('documents_in_progress').insert(doc_progress_payload).execute()
