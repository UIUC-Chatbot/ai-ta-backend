    ...
    try:
      ...
      if rtag.get('target'):
        ...
        if is_expansion_string(backward_between_span):
          ...
          surface_num_range = _get_surface_range(rtag.find_previous_sibling('ref').text.strip(), surface_span)
...
def process_paragraph(sp: BeautifulSoup, para_el: bs4.element.Tag, section_names: List[Tuple], bib_dict: Dict,
                      ref_dict: Dict, bracket: bool) -> Dict:
  """
  ...
  if not para_el.text:
    return {'text': "", 'cite_spans': [], 'ref_spans': [], 'eq_spans': [], 'section': section_names}
  ...
  for span in re.finditer(r'(CITETOKEN\d+)', para_text):
    ...
    all_spans_to_replace.append((span.start(), span.start() + len(uniq_token), uniq_token, surface_text))
...
def extract_abstract_from_tei_xml(sp: BeautifulSoup, bib_dict: Dict, ref_dict: Dict,
                                  cleanup_bracket: bool) -> List[Dict]:
  """
      ...
      for div in sp.abstract.find_all('div'):
        if div.text:
          if div.p:
            ...
            if div.text:
              abstract_text.append(process_paragraph(sp, div, [(None, "Abstract")], bib_dict, ref_dict,
                                                     cleanup_bracket))
    # process all paragraphs
    elif sp.abstract.p:
      for para in sp.abstract.find_all('p'):
        if para.text:
          abstract_text.append(process_paragraph(sp, para, [(None, "Abstract")], bib_dict, ref_dict, cleanup_bracket))
...
def extract_body_text_from_div(sp: BeautifulSoup, div: bs4.element.Tag, sections: List[Tuple], bib_dict: Dict,
                               ref_dict: Dict, cleanup_bracket: bool) -> List[Dict]:
  """
  ...
  if div.div:
    for subdiv in div.find_all('div'):
      ...
      if subdiv.head:
        chunks += extract_body_text_from_div(sp, subdiv,
                                             sections + [(subdiv.head.get('n', None), subdiv.head.text.strip())],
                                             bib_dict, ref_dict, cleanup_bracket)
        subdiv.head.decompose()
      # no header, process with same section list
      else:
        chunks += extract_body_text_from_div(sp, subdiv, sections, bib_dict, ref_dict, cleanup_bracket)
  # process tags individuals
  for tag in div:
    try:
      if tag.name == 'p':
        if tag.text:
          chunks.append(process_paragraph(sp, tag, sections, bib_dict, ref_dict, cleanup_bracket))
...
def extract_body_text_from_tei_xml(sp: BeautifulSoup, bib_dict: Dict, ref_dict: Dict,
                                   cleanup_bracket: bool) -> List[Dict]:
  """
    Parse body text from soup
    :param sp:
...
def extract_back_matter_from_tei_xml(sp: BeautifulSoup, bib_dict: Dict, ref_dict: Dict,
                                     cleanup_bracket: bool) -> List[Dict]:
  """
...
def convert_tei_xml_soup_to_s2orc_json(soup: BeautifulSoup, paper_id: str, pdf_hash: str) -> Paper:
  ...
  return Paper(paper_id=paper_id,
               pdf_hash=pdf_hash,
               metadata=metadata,
